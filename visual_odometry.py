...
kMinNumFeature = 1000
folder = '/Users/farhantoddywala/desktop/preprocessed_imgs/03-0-2/'
...
class VisualOdometry:
    ...
    def getAbsoluteScale(self, frame_id):
        return self.scale  #specialized for KITTI odometry dataset#return 1 #ss = self.annotations[frame_id-1].strip().split()
        '''y_prev = float(ss[7])
        z_prev = float(ss[11])
    ...
    def processFirstFrame(self, frame_id):
        ...
        #print(frame_id)
        #name = folder + str(frame_id) + "_interest_points"
        #self.px_ref = np.loadtxt(name).astype("float32")
    ...
    def processFrame(self, frame_id):
        ...
        if(self.px_ref.shape[0] < kMinNumFeature):
            #name = folder + str(frame_id) + "_interest_points"
            #self.px_cur = np.loadtxt(name).astype("float32")
            #print("reset")
